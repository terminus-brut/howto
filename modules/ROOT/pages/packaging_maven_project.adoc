...
xref:common_errors.adoc#error_missing_dependency[missing dependencies]
...
`pom.xml` files. See xref:pom_macros.adoc[Macros for POM modification]
section for more information.

Now try to build the project again. The build will fail with
xref:common_errors.adoc#error_compilation_failure[compilation failure].
...
xref:introduction_for_developers.adoc#querying_repositories[query repositories].
======
...
See xref:mvn_macros.adoc#mvn_alias[Additional Mappings]
for more information on `%mvn_alias`.
